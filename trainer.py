            ...
            for i, (images, original_size, image_name) in enumerate(tqdm(self.test_loader)):
                ...
                for i in range(images.size(0)):
                    ...
                    if self.args.save_map is not None:
                    	...
                    	print(os.path.join(self.te_img_folder_pred,image_name[i]+'.png'))
                    	img = cv2.cvtColor(cv2.imread(os.path.join(self.te_img_folder_pred,image_name[i]+'.png')),cv2.COLOR_BGR2RGB)
